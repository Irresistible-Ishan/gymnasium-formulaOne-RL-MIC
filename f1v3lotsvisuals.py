...
# ==========================================
# 2. THE CUSTOM RENDER CALLBACK
# ==========================================
class RenderCallback(BaseCallback):
    """
    Custom callback that pauses training every `render_freq` steps to 
    show you a visual test run of what the AI has learned so far.
    """
    def __init__(self, render_env, render_freq=10000, verbose=0):
        super(RenderCallback, self).__init__(verbose)
        self.render_env = render_env
        self.render_freq = render_freq

    def _on_step(self) -> bool:
        # Check if we hit the interval (e.g., every 10,000 steps)
        if self.n_calls % self.render_freq == 0:
            print(f"\n--- PAUSING TRAINING: Showing Visual Test at Step {self.n_calls} ---")
            
            # Run one visual episode using the current state of the model
            obs, info = self.render_env.reset()
            terminated = False
            truncated = False
            
            while not (terminated or truncated):
                # We use deterministic=True to see its true learned behavior, not random exploration
                action, _states = self.model.predict(obs, deterministic=True)
                obs, reward, terminated, truncated, info = self.render_env.step(action)
                self.render_env.render()
                
            print("--- TEST FINISHED: Resuming Background Training... ---\n")
            
            # Keep the window slightly open for a moment so you see the crash/finish
...
if __name__ == "__main__":
    ...
    # The environment used for raw speed training
    train_env = FastTrackEnv()
    
    # A separate environment purely used to pop open the window for the callback
    visual_env = FastTrackEnv(render_mode="human")
    
    # Create the callback, telling it to pop up the window every 10,000 steps
    visual_callback = RenderCallback(render_env=visual_env, render_freq=10000)
    ...
    model.learn(total_timesteps=50000, callback=visual_callback)
